...
$renderFailure = static function (string $message = 'File Manager could not start.') use ($requestId): never {
    if (!headers_sent()) {
...
register_shutdown_function(static function () use ($writeVaultLog, $renderFailure): void {
    ...
    if (!headers_sent()) {
        $renderFailure('A PHP fatal error occurred while loading File Manager.');
    }
});
...
try {
    ...
    $message = 'File Manager bootstrap failed.';
