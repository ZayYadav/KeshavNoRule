...
public final class TeamDark8 {
    ...
    private static boolean verifyTrustedServerLoader(Context context) throws Exception {
        ...
        File expected = TeamDark5.trustedLoaderFile(context);

        if (!TeamDark5.TRUSTED_LOADER_NAME.equals(loader.getName())) return false;
        ...
        TeamDark6 secure = new TeamDark6(context);
        String expectedHash = secure.getSt(TeamDark5.LOADER_HASH_KEY, "");
        String expectedSize = secure.getSt(TeamDark5.LOADER_SIZE_KEY, "");
        ...
        String actualHash = TeamDark5.sha256File(loader);
        ...
        try {
            return TeamDark2.nativeVerifyServerLoader(
                    context,
                    expectedHash,
                    boundSize);
        } catch (Throwable ignored) {
