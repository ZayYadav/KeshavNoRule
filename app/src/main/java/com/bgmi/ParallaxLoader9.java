...
@Obfuscate
public final class ParallaxLoader9 {
    ...
    public static Runnable installRuntimeGuard(Activity activity, Handler handler) {
        Runnable guard = new Runnable() {
            ...
            public void run() {
                ...
                boolean ok = false;
                try {
                    ok = ParallaxLoader8.verify(activity)
                            && ParallaxLoader2.nativeVerifySignature(activity)
                            && ParallaxLoader2.nativeCustomIntegrity(activity);
                } catch (Throwable ignored) {
                    ok = false;
                }
