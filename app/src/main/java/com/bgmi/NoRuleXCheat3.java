...
import com.bgmi.utils.NoRuleXCheat4;
...
@Obfuscate
public class NoRuleXCheat3 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            nativeIntegrityOk = NoRuleXCheat2.nativeVerifySignature(this)
                    && NoRuleXCheat2.nativeCustomIntegrity(this);
        } catch (Throwable ignored) {
        ...
        if (btnStart != null) {
            NoRuleXCheat7.applyTouchBounce(btnStart, () -> {
                NoRuleXCheat7.getInstance().playLaunch();
                handleStart();
            });
        }
    ...
    private boolean ensureSdkActivatedThenContinue() {
        ...
        final String sdkKey;
        try {
            sdkKey = NoRuleXCheat1.getSdkKey();
        ...
        sdkActivationHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                if (activated) {
                    sdkActivationPending.set(false);
                    sdkActivationHandler.removeCallbacksAndMessages(null);
                    Toast.makeText(NoRuleXCheat3.this, "SDK Activated", Toast.LENGTH_SHORT).show();
                    handleStartAfterSdkReady();
                    return;
                }
                ...
                if (SystemClock.elapsedRealtime() >= deadline) {
                    ...
                    NoRuleXCheat7.getInstance().playError();
                    Toast.makeText(NoRuleXCheat3.this, message, Toast.LENGTH_LONG).show();
                    return;
                }
    ...
    private void forceAutoCopyObb() {
        ...
        timerHandler.postDelayed(() -> {
            if (!isFinished.get()) {
                isFinished.set(true);
                Toast.makeText(NoRuleXCheat3.this, "Copy Timeout! Check manually.", Toast.LENGTH_LONG).show();
            }
        }, 60000);
            ...
            try {
                ...
                if (sourceFiles == null || sourceFiles.length == 0) {
                    if (!isFinished.get()) {
                        ...
                        runOnUiThread(() -> {
                            NoRuleXCheat7.getInstance().playError();
                            Toast.makeText(NoRuleXCheat3.this, "Source OBB missing!", Toast.LENGTH_LONG).show();
                        });
                    }
                    return;
                }
                ...
                if (!isFinished.get()) {
                    isFinished.set(true);
                    runOnUiThread(() -> {
                        Toast.makeText(NoRuleXCheat3.this, "OBB Ready! Launching...", Toast.LENGTH_SHORT).show();
                        launchGame();
                    });
                }
            } catch (Exception e) {
                if (!isFinished.get()) {
                    ...
                    runOnUiThread(() -> {
                        NoRuleXCheat7.getInstance().playError();
                        Toast.makeText(NoRuleXCheat3.this, "Error: " + e.getMessage(), Toast.LENGTH_LONG).show();
                    });
                }
            }
        }).start();
                ...
                try {
                    ...
                    if (expiry != null) {
                        ...
                        if (diff > 0) {
                            ...
                            Toast.makeText(NoRuleXCheat3.this, "Subscription Expired!", Toast.LENGTH_SHORT).show();
                            finish();
                        }
                    }
                } catch (Throwable ignored) {
