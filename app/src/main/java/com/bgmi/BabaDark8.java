...
@Obfuscate
public final class BabaDark8 {

    private static final Set<String> ALLOWED_NATIVE_LIBS = new HashSet<>();

    static {
        ALLOWED_NATIVE_LIBS.add("libBabaDarkLoader.so");
    ...
    private static boolean verifyApkNativeEntries(Context context) throws Exception {
        ...
        return found.size() == 2
                && found.contains("libBabaDarkLoader.so")
    ...
    private static boolean verifyTrustedServerLoader(Context context) throws Exception {
        ...
        File expected = BabaDark5.trustedLoaderFile(context);

        if (!BabaDark5.TRUSTED_LOADER_NAME.equals(loader.getName())) return false;
        ...
        BabaDark6 secure = new BabaDark6(context);
        String expectedHash = secure.getSt(BabaDark5.LOADER_HASH_KEY, "");
        String expectedSize = secure.getSt(BabaDark5.LOADER_SIZE_KEY, "");
        ...
        String actualHash = BabaDark5.sha256File(loader);
        ...
        try {
            return BabaDark2.nativeVerifyServerLoader(
                    context,
                    expectedHash,
                    boundSize);
        } catch (Throwable ignored) {
