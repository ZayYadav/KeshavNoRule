...
@Obfuscate
public final class BabaDark9 {

    private static final AtomicBoolean SHOWING = new AtomicBoolean(false);

    // Keep the same integrity-failure behavior used by TeamDarkLoader while
    // preserving BABATILLU branding and its own signing identity.
    ...
    private static final long RUNTIME_GUARD_INTERVAL_MS = 3_000L;
            ...
            try {
                ...
                if (detailView != null) {
                    detailView.setText(detail == null || detail.trim().isEmpty()
                            ? "BABA DARK security detected an unauthorized APK or runtime modification."
                            : detail);
                }
    ...
    public static Runnable installRuntimeGuard(Activity activity, Handler handler) {
        Runnable guard = new Runnable() {
            ...
            public void run() {
                ...
                boolean ok = false;
                try {
                    ok = BabaDark8.verify(activity)
                            && BabaDark2.nativeVerifySignature(activity)
                            && BabaDark2.nativeCustomIntegrity(activity);
                } catch (Throwable ignored) {
                    ok = false;
                }
                ...
                try {
                    ...
                    // Activity may already be shutting down.
                }
            }
        };
    ...
    private static void startIntegrityAudio(Context context, long session) {
        ...
        Thread worker = new Thread(() -> {
            File audioFile = null;
            try {
                File cacheDir = new File(context.getCacheDir(), "babadark_integrity");
                        ...
                        public void run() {
                            ...
                            try {
                                if (integrityPlayer != null) {
                                    // Player gain only; never override the user's system volume.
    ...
    private static void downloadSilently(String source, File target) throws Exception {
        ...
        for (int redirects = 0; redirects < 6; redirects++) {
            ...
            try {
                ...
                connection.setRequestProperty("User-Agent", "Mozilla/5.0 Android BabaDark");
