...
public class PantherLoaderUpdater extends AsyncTask<String, Integer, String> {
    ...
    // Existing server/download contract intentionally unchanged.
    ...
    private static final String PREF_NAME = "com.dark.panther.download";
    ...
    private volatile boolean operationOk;

    public PantherLoaderUpdater(Context context) { this(context, null); }
    public PantherLoaderUpdater(Context context, Callback callback) {
    ...
    protected String doInBackground(String... params) {
        ...
        try {
            ...
            if (serverVersion == null || serverVersion.length() > 128) return "Version check failed";
            ...
            clearOldLoaderState();
            String error = downloadAndExtract(downloadUrl());
    ...
    protected void onPostExecute(String result) {
        if (callback != null) callback.onComplete(operationOk);
    }

    private boolean isHttps(URL url) { return url != null && "https".equalsIgnoreCase(url.getProtocol()); }

    private void clearOldLoaderState() {
        safeDelete(new File(context.getFilesDir(), ZIP_NAME));
        File loaderDir = new File(context.getFilesDir(), "loader");
        if (loaderDir.exists()) deleteSoFilesRecursively(loaderDir);
        try {
    ...
    private String downloadAndExtract(String urlString) {
        ...
        try {
            ...
            if (!isHttps(url)) return "HTTPS required";
            ...
            if (!isHttps(con.getURL())) return "Unsafe redirect rejected";
            if (code < 200 || code >= 300) return "HTTP Error: " + code;

            long total = con.getContentLengthLong();
            if (total > MAX_DOWNLOAD_BYTES) return "Download too large";

            long done = 0;
            try (InputStream in = con.getInputStream(); FileOutputStream out = new FileOutputStream(zipFile)) {
                byte[] buf = new byte[8192];
                int len;
                while ((len = in.read(buf)) != -1) {
                    done += len;
                    if (done > MAX_DOWNLOAD_BYTES) return "Download limit exceeded";
                    out.write(buf, 0, len);
            ...
            if (!targetDir.exists() && !targetDir.mkdirs()) return "Could not create loader directory";
            ...
            if (loader.length() > MAX_DOWNLOAD_BYTES) return "Trusted loader exceeds size limit";
    ...
    private void unzipSafely(File zipFile, File targetDir) throws Exception {
        ...
        try (ZipInputStream zis = new ZipInputStream(new FileInputStream(zipFile))) {
            ...
            while ((entry = zis.getNextEntry()) != null) {
                ...
                if (entryName == null || entryName.isEmpty()) throw new SecurityException("Bad zip entry");
                File outFile = new File(targetDir, entryName);
                String canonical = outFile.getCanonicalPath();
                if (!canonical.startsWith(targetRoot)) throw new SecurityException("Zip traversal");

                if (!entry.isDirectory() && entryName.toLowerCase(Locale.US).endsWith(".so")
                        && !TRUSTED_LOADER_NAME.equals(outFile.getName())) {
                    throw new SecurityException("Unexpected shared library");
                }

                if (entry.isDirectory()) {
                    if (!outFile.exists() && !outFile.mkdirs()) throw new SecurityException("Directory create failed");
                } else {
                    File parent = outFile.getParentFile();
                    if (parent != null && !parent.exists() && !parent.mkdirs()) {
                        throw new SecurityException("Parent create failed");
                    }
                    try (FileOutputStream fos = new FileOutputStream(outFile)) {
                        int count;
                        while ((count = zis.read(buffer)) != -1) {
                            extracted += count;
                            if (extracted > MAX_EXTRACTED_BYTES) throw new SecurityException("Archive too large");
    ...
    private File normalizeTrustedLoader(File targetDir) throws Exception {
        ...
        if (soFiles.size() != 1) throw new SecurityException("Unexpected loader library count");
        File found = soFiles.get(0);
        if (!TRUSTED_LOADER_NAME.equals(found.getName())) throw new SecurityException("Unexpected loader name");

        File expected = new File(targetDir, TRUSTED_LOADER_NAME);
        String expectedPath = expected.getCanonicalPath();
        if (!found.getCanonicalPath().equals(expectedPath)) {
        ...
        if (!expected.getCanonicalPath().equals(expectedPath)) throw new SecurityException("Loader path normalization failed");
        return expected;
    }
    ...
    private void deleteSoFilesRecursively(File dir) {
        ...
        if (files == null) return;
        for (File file : files) {
            if (file.isDirectory()) deleteSoFilesRecursively(file);
            else if (file.getName().toLowerCase(Locale.US).endsWith(".so")) safeDelete(file);
        }
    }
    ...
    private boolean verifyStoredFingerprint(File loader) {
        try {
            ...
            if (!loader.getCanonicalPath().equals(trustedLoaderFile(context).getCanonicalPath())) return false;
            PantherPrefs secure = new PantherPrefs(context);
    ...
    private String getServerVersion() {
        ...
        try {
            ...
            con.connect();
            int code = con.getResponseCode();
            if (!isHttps(con.getURL()) || code < 200 || code >= 300) return null;
